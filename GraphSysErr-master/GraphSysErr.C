...
# endif
#ifndef DEFAULT_VIZ
# define DEFAULT_VIZ __attribute__ ((visibility("default")))
#endif
...
public:
  ...
  void Scale(Double_t s)
  {
    ...
    for (Int_t i = 0; i < GetN(); i++) ScalePoint(i, s);
  }
  void ScalePoint(Int_t i, Double_t s)
  {
    Double_t y = GetY(i);
    ...
    while ((p2p = static_cast<HolderP2P*>(pNext()))) {
      Double_t fac = (p2p->IsRelative() ? (TMath::Abs(y) < 1e-9) ? 0 : 1/y : s);
  ...
  {
    // Printf("Swapping points %d and %d total %d %s",
    //        i, j, GetN(), reflect ? "(reflect)" : "");
  ...
  GraphSysErr* Reflect(Double_t x0=0) const
  {
    ...
    if (GetN() % 2 == 1) { // Uneven number of points!
      cpy->SwapPoints(GetN()/2,GetN()/2,true);
    }
    return cpy;
  }
   ...
   * @param sep   If true, collect all systematic uncertainies
   * 
   ...
   * @param nxsign Number of significant digits on abscissa
   *
   ...
   * @param option Options
   * @param nsign  Number of significant digits on ordinates
   * @param nxsign Number of significant digits on abscissa
   *
   * - H Export file header 
   * - C Export file comment 
   ...
   */
  struct DEFAULT_VIZ Holder : public TNamed, TAttLine, TAttFill 
  {
   ...
   * 
   */
  struct DEFAULT_VIZ HolderCommon : public Holder 
  {
    /** Containing class is a friemd */
    friend struct GraphSysErr;
    /** 
...
protected:
   ...
   * @param nsign  Number of significant digits on ordinate
   * @param nxsign Number of significant digits on abscissa
   * 
   * @return output stream 
   */
...
#ifndef  GRAPHSYSERR_INIT
# define GRAPHSYSERR_INIT
UInt_t GraphSysErr::kVerbose = 0;
#endif 
/** 
